...
def index_directory(
    ...
    """Index all supported files under kb_dir. Returns total chunks added."""
    total = 0
    ...
    for path in sorted(files):
        ...
        if not text.strip():
            ...
            continue
        n = vector_store.add_document(source_file=path.name, text=text, config=DEFAULT_CHUNKING)
        total += n
    return total
...
def main() -> int:
    ...
    parser.add_argument("--kb-dir", type=Path, default=Path("data/kb"), help="Knowledge-base directory")
